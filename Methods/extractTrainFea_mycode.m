function [C, A, Out] = extractTrainFea(trainTensor,model, opts)
%EXTRACTTRAINFEA 提取训练集特征
%   trainTensor: 训练集数据张量
...
Mnrm = norm(trainTensor)/train_num;
...
%             C = updateC_L2(trainTensor,C,A);
    elseif model == "L2"
        C = updateC_L2(trainTensor,C,A);
    elseif model == "L1"
        C = updateC_L1(trainTensor,C,A);
    end
    % ----- update Factor Matrix ------
    for mat_md = 1:N
        if model == "Linf"
            A = updateAn_Linf(trainTensor, C, A, mat_md);
        elseif model == "L1"
%             A = updateAn_L1(trainTensor, C, A, mat_md);
        elseif model == "L2"
            A = updateAn_L2(trainTensor, C, A, mat_md);
        end
    end
