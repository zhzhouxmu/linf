function [An] = updateAn_L1(data_set,core,An,mat_md)
% update one mode factor matrix for L1 model
...
cvx_begin
    ...
    variable t(num_train);
    minimize(sum(t));
    subject to
        for i = 1:num_train
            norm(mat_X(:,:,i) - ...\
                reshape(x,[size_Ai(1),size_Ai(2)])* GnA(:,:,i),'fro') <= t(i);
        end
cvx_end
